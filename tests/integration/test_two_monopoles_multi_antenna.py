...
def test_two_monopoles_vs_dipole_gold_standard(solver_running):
    ...
    directivity_linear_dipole, directivity_dBi_dipole, U_pattern_dipole, max_indices_dipole = (
        compute_directivity_from_pattern(E_theta_dipole, E_phi_dipole, theta_range, phi_range)
    )
    ...
    print("\nUpper Monopole:")
    print(
        f"  Nodes: {len(nodes_upper)} (z: {nodes_upper[0,2]*100:.2f} to {nodes_upper[-1,2]*100:.2f} cm)"
    )
    print(f"  Edges: {len(edges_upper)}")
    print("  Source: Node 0 -> Node 1, V = +1.0 V")

    print("\nLower Monopole:")
    print(
        f"  Nodes: {len(nodes_lower)} (z: {nodes_lower[0,2]*100:.2f} to {nodes_lower[-1,2]*100:.2f} cm)"
    )
        ...
        "antennas": [
            {
                ...
                "radii": radii_upper,
                "voltage_sources": [
                    {"node_start": 0, "node_end": 1, "value": 1.0}  # Ground  # First node
                ],
                "current_sources": [],
                "loads": [],
            },
    ...
    response = requests.post(f"{BASE_URL}{API_PREFIX}/solve/multi", json=request_data, timeout=60)
    ...
    assert (
        impedance_diff_percent < 50
    ), f"Impedance difference {impedance_diff_percent:.1f}% exceeds 50%"

    if impedance_diff_percent < 20:
        print("  [PASS] PASS: Impedances match within 20%")
    else:
        print("  [PASS] PASS: Impedances reasonably close (within 50%)")
    ...
    currents_upper = np.array(
        [complex(c) if isinstance(c, str) else c for c in sol_upper["branch_currents"]]
    )
    currents_lower = np.array(
        [complex(c) if isinstance(c, str) else c for c in sol_lower["branch_currents"]]
    )
    ...
    assert (
        current_rms_error_percent < 50
    ), f"Current RMS error {current_rms_error_percent:.1f}% exceeds 50%"

    if current_rms_error_percent < 20:
        print("  [PASS] PASS: Current distributions match within 20%")
    else:
        print("  [PASS] PASS: Current distributions reasonably close (within 50%)")
    ...
    if directivity_error < 0.5:
        print("  [PASS] PASS: Directivity matches theoretical (within 0.5 dB)")
    else:
    ...
    print(" TEST SUMMARY")
    print("=" * 80)
    print("  [PASS] Impedance: Two monopoles match dipole within tolerance")
    print("  [PASS] Current: Distribution matches within tolerance")
    print("  [PASS] Directivity: Single dipole matches theoretical value")
    print("\n  Multi-antenna solver correctly combines separate antennas!")
    print("=" * 80)
