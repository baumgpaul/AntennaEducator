...
from backend.postprocessor.pattern import compute_radiation_intensity, compute_total_radiated_power
from backend.solver.solver import compute_radiation_efficiency, solve_peec_frequency_sweep
...
class TestSolverFieldIntegration:
    ...
    def test_dipole_complete_analysis(self):
        ...
        efficiency = compute_radiation_efficiency(sol.accepted_power, sol.power_dissipated, P_rad)
    ...
    def test_monopole_ground_plane(self):
        ...
        E_field, H_field = compute_far_field(
            frequencies, np.array([sol.branch_currents[: len(edges)]]), nodes, edges, theta, phi
        )
    ...
    def test_power_conservation(self):
        ...
        # Solve
        result = solve_peec_frequency_sweep(nodes, edges, radii, frequencies, vsources)
...
class TestEdgeCases:
    ...
    def test_multiple_edges(self):
        ...
        # 3-segment dipole
        nodes = np.array([[0, 0, 0], [0, 0, 0.166], [0, 0, 0.333], [0, 0, 0.5]])
        edges = [[1, 2], [2, 3], [3, 4]]
        ...
        E_field, _ = compute_far_field(
            frequencies, np.array([sol.branch_currents[:3]]), nodes, edges, theta, phi
        )
