...
def test_incremental_field_computation():
    ...
    assert (
        field1_data["E_magnitudes"] != field2_data["E_magnitudes"]
    ...
    assert (
        directivity_response.status_code == 200
    ), f"Directivity failed: {directivity_response.text}"
    ...
    print(
        f"   ✓ Pattern points: {len(directivity_data['theta_angles'])} × {len(directivity_data['phi_angles'])}"
    )
