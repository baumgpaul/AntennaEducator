/**
 * VoltagePlot - Line chart showing voltage magnitude and phase vs frequency
 * Displays data for a specific port number
 */
...
interface VoltagePlotProps {
  data: VoltagePoint[];
  portNumber: number;
  title?: string;
}

function VoltagePlot({ data, portNumber, title }: VoltagePlotProps) {
    ...
    return (
      <Box sx={{ p: 2, textAlign: 'center' }}>
        <Typography variant="body2" color="text.secondary">
          No voltage data available for Port {portNumber}
        </Typography>
      </Box>
    );
  }

  const plotTitle = title || `Voltage (Port ${portNumber})`;
    ...
    <Box sx={{ width: '100%', height: 350, p: 2 }}>
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 20 }}>
          ...
          <YAxis
            yAxisId="left"
            label={{ value: 'Magnitude (V)', angle: -90, position: 'insideLeft' }}
          />
          <YAxis
            yAxisId="right"
            orientation="right"
