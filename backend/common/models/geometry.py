...
class Source(BaseModel):
    ...
    type: Literal["voltage", "current"] = Field(description="Type of source excitation")
    amplitude: complex = Field(description="Source amplitude (can be complex for phase)")
    ...
    node_end: Optional[int] = Field(
        default=None, description="Ending node index: 1-based for mesh nodes (1 to N), 0 for ground"
    )

    # Series impedance for voltage sources (matches PEEC Voltage_Source structure)
    series_R: float = Field(default=0.0, description="Series resistance in Ohms", ge=0)
    series_L: float = Field(default=0.0, description="Series inductance in Henries", ge=0)
...
class LumpedElement(BaseModel):
    ...
    R: float = Field(default=0.0, description="Resistance in Ohms", ge=0)
    L: float = Field(default=0.0, description="Inductance in Henries", ge=0)
    C_inv: float = Field(
        default=0.0,
...
class Mesh(BaseModel):
    ...
    nodes: List[List[float]] = Field(description="Node coordinates [N x 3] array as list")
    edges: List[List[int]] = Field(description="Edge connectivity [M x 2] array (node indices)")
    radii: List[float] = Field(description="Wire radius for each edge [M] array")
    edge_to_element: Dict[int, str] = Field(
        default_factory=dict, description="Mapping from edge index to element ID"
    )
    source_edges: List[int] = Field(
        default_factory=list, description="List of edge indices that have sources"
    )
