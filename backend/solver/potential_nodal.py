...
def build_capacitive_elements(
    edges: List[EdgeGeometry], edge_list: List[List[int]], radii: np.ndarray, n_nodes: int
    ...
    if len(edges) != len(edge_list):
        raise ValueError(
            f"Mismatch: {len(edges)} EdgeGeometry objects but {len(edge_list)} edge connections"
        )
...
def build_capacitive_element_vectors(
    edges: List[EdgeGeometry], edge_list: List[List[int]], radii: np.ndarray, n_nodes: int
...
def compute_nodal_potential_coefficient(
    X: np.ndarray, Y: np.ndarray, gauss_points: np.ndarray, gauss_weights: np.ndarray
) -> float:
    """
...
def assemble_nodal_potential_matrix(
