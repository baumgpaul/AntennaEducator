...
def compute_self_inductance(length: float, radius: float) -> float:
    ...
    if radius >= length:
        raise ValueError(
            f"Radius {radius} must be less than length {length} (thin wire approximation)"
        )
...
def compute_mutual_inductance_1d(
    edge1: EdgeGeometry, edge2: EdgeGeometry, gauss_points: np.ndarray, gauss_weights: np.ndarray
) -> float:
    """
    ...
    gauss_positions = (
        Y_center[np.newaxis, :] + gauss_points[:, np.newaxis] * half_vec[np.newaxis, :]
    )
...
def assemble_inductance_matrix(
    ...
    for i in range(n_edges):
        ...
        for j in range(i + 1, n_edges):
            M_ij = compute_mutual_inductance_1d(edges[i], edges[j], gauss_points, gauss_weights)
            L[i, j] = M_ij
            L[j, i] = M_ij  # Symmetry
