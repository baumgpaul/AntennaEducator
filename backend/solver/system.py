...
def build_appended_incidence_matrix(
    voltage_sources: List[VoltageSource], loads: List[Load], n_edges: int
...
def solve_peec_system(
    system_matrix: np.ndarray, source_vector: np.ndarray, n_branches: int
...
def compute_node_voltages(
    I: np.ndarray, I_source: np.ndarray, A: np.ndarray, P: np.ndarray, omega: float
) -> np.ndarray:
    """
