...
GAUSS_LEGENDRE_DATA = {
    ...
    2: {"points": np.array([-0.57735027, 0.57735027]), "weights": np.array([1.0, 1.0])},
    3: {
    ...
    6: {
        ...
        "weights": np.array(
            [0.17132449, 0.36076157, 0.46791394, 0.46791394, 0.36076157, 0.17132449]
        ),
    },
    7: {
        "points": np.array(
...
def get_gauss_points(n: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if n not in GAUSS_LEGENDRE_DATA:
        ...
        raise ValueError(f"Unsupported quadrature order: {n}. " f"Available orders: {available}")
