...
def compute_total_radiated_power(
    radiation_intensity: np.ndarray,
    ...
    dphi = np.diff(phi_angles).mean() if len(phi_angles) > 1 else 0
...
def compute_principal_plane_patterns(
    ...
    # E-plane pattern (φ = 0°)
    E_plane_pattern = np.sqrt(np.abs(E_theta[:, phi_0_idx])**2 + 
                              np.abs(E_phi[:, phi_0_idx])**2)
    
    # H-plane pattern (φ = 90°)
    H_plane_pattern = np.sqrt(np.abs(E_theta[:, phi_90_idx])**2 + 
                              np.abs(E_phi[:, phi_90_idx])**2)
