...
from backend.postprocessor.pattern import compute_directivity, compute_total_radiated_power, compute_radiation_intensity
...
def test_current_source_dipole_golden_standard():
    ...
    assert element.sources[0].node_start == 1, f"Source 1 should be at node 1, got {element.sources[0].node_start}"
    assert element.sources[1].node_start == 7, f"Source 2 should be at node 7, got {element.sources[1].node_start}"
    ...
    print(f"  Expected relationship: approximately equal magnitude, opposite sign")
    ...
    if symmetry_error < 0.01:
        ...
        print(f"  ⚠ Pattern may not be symmetric (expected for z-oriented dipole)")
...
if __name__ == "__main__":
    ...
    print("="*80)
    print(f"\nSummary:")
    print(f"  Current source implementation: ✓ VERIFIED")
    print(f"  Balanced feed configuration: ✓ VERIFIED")
    print(f"  Directivity computation: ✓ VERIFIED")
